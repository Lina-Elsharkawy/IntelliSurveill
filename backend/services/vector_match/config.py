...
T_IDENTIFY = float(os.getenv("T_IDENTIFY", "0.62"))      # accept known if best_sim >= this and margin ok
T_AUTOLEARN = float(os.getenv("T_AUTOLEARN", "0.75"))    # auto-learn only if best_sim >= this (stricter)
M_MARGIN = float(os.getenv("M_MARGIN", "0.03"))          # require best_sim - second_sim >= this
MIN_QUALITY = float(os.getenv("MIN_QUALITY", "0.55"))     # if you provide quality_score, enforce it
...
SEARCH_AUTHORITATIVE_FIRST = os.getenv("SEARCH_AUTHORITATIVE_FIRST", "true").lower() == "true"
