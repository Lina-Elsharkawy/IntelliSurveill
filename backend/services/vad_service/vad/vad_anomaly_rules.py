...
_VLM_TO_ANOMALY_EVENT: dict[str, set[str]] = {
    ...
    "possible_intrusion_or_security_event": {"intrusion", "after_hours"},
    "unsafe_equipment_interaction":         {"other"},
...
def load_anomaly_rules(conn: Any) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        rule_type  = str(r.get("rule_type") or "trigger").lower()
...
def deterministic_rule_matches(
    *,
    ...
    for rule in rules:
        ...
        if all_ets and not any(_event_type_matches(et, vlm_event_type) for et in all_ets):
            continue
