...
def infer_label(row: pd.Series) -> int:
    """
    Safe label inference for this dataset.

    IMPORTANT:
    Do NOT classify by searching the whole path for the word "anomaly",
    because the parent folder is named "anomaly_dataset". That would wrongly
    label Dataset\\Normal\\video*.mp4 as anomaly.

    Priority:
      1) explicit numeric label columns if present
      2) video_id prefixes: Normal__ / Anomaly__
      3) exact dataset folder markers: /dataset/normal/ and /dataset/anomaly/
      4) exact path parts: normal / anomaly
    """
    # 1) Explicit numeric label, if present.
    ...
    video_id = str(row.get("video_id", "")).strip().lower()
    video_path = str(row.get("video_path", row.get("source_video", ""))).strip().lower().replace("\\", "/")

    # 2) The feature metadata uses these clean prefixes.
    if video_id.startswith("normal__"):
        return 0
    if video_id.startswith("anomaly__") or video_id.startswith("abnormal__"):
        return 1

    # 3) Exact dataset-folder markers.
    if "/dataset/normal/" in video_path:
        return 0
    if "/dataset/anomaly/" in video_path or "/dataset/abnormal/" in video_path:
        return 1

    # 4) Exact path parts only; do not match "anomaly_dataset".
    parts = [p for p in video_path.split("/") if p]
    if "normal" in parts:
        return 0
    if "anomaly" in parts or "abnormal" in parts:
        return 1

    raise ValueError(
        f"Could not infer label safely. video_id={row.get('video_id', '')!r}, "
        f"video_path={row.get('video_path', row.get('source_video', ''))!r}"
    )
