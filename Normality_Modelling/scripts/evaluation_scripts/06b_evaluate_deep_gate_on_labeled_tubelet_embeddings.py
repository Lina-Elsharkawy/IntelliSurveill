...
def load_threshold(thresholds: dict, k: int, threshold_key: str) -> float:
    ...
    raise KeyError(f"Threshold key={threshold_key!r} for k={k} not found")
...
def main() -> None:
    ...
    thresholds_path = args.artifact_dir / "04_thresholds.json"
    ...
    selected_threshold = load_threshold(thresholds, args.selected_k, args.threshold_key)
    ...
    for k in k_values:
        ...
        for score_col in score_cols:
            ...
            for threshold_key in sorted((thresholds.get(f"k{k}") or {}).keys()):
                th = load_threshold(thresholds, k, threshold_key)
    ...
    print(f"Selected threshold: {args.threshold_key} = {selected_threshold:.8f}")
