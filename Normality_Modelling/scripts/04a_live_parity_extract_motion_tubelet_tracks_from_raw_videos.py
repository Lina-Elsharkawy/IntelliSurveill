...
motion tubelet track metadata using the same tracking/sampling order as the
live RTSP Pose gate: track every decoded frame, then sample tracked outputs.
...
Notes
-----
- This live-parity pose-focused version runs YOLO.track() on every decoded frame.
- It only appends detections to tubelet buffers at 5 FPS by default.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="YOLO detection confidence passed to model.track(). Live Pose parity default: 0.25.",
    )
    parser.add_argument(
        "--min_conf",
        type=float,
        default=0.25,
        help=(
            "Minimum confidence allowed inside an accepted tubelet. "
            "For live parity this should not be stricter than --det_conf."
        ),
    )
    parser.add_argument("--min_bbox_width", type=float, default=0.0)
    parser.add_argument("--min_bbox_height", type=float, default=0.0)
    parser.add_argument("--min_mean_iou", type=float, default=0.0)
    parser.add_argument("--max_center_jump_ratio", type=float, default=float("inf"))
    parser.add_argument("--max_track_gap", type=int, default=0)
    parser.add_argument(
        "--allow_sample_gaps",
        action="store_true",
        help=(
...
def reset_yolo_tracker_state(model: YOLO) -> None:
    ...
    Why this matters:
    We call model.track() on every decoded frame with persist=True so IDs remain stable
    ...
    try:
        ...
        with pbar_context as pbar:
            while frame_count <= 0 or frame_index < frame_count:
                # LIVE-PARITY BEHAVIOR:
                # Decode/retrieve every frame and run YOLO.track() on every decoded frame.
                # Only after tracking do we decide whether this frame should contribute a
                # 5 FPS sample to the tubelet buffer. This matches the live RTSP script and
                # avoids starving ByteTrack/BoT-SORT by calling track() only on sampled frames.
                ok, frame = cap.read()
                if not ok or frame is None:
                    break
                ...
                is_sample_frame = (current_frame_index % frame_step) == 0
                ...
                detections = extract_detections_from_result(results[0]) if results else []

                # Track IDs are observed on every decoded frame, but tubelet buffers are
                # intentionally updated only on sampled frames.
                ...
                for track_id, conf, bbox in detections:
                    ...
                    for window in buffers[track_id].candidate_windows(args.tubelet_frames, args.stride):
                        ...
                        record = {
                            "schema_version": "motion_tubelet_tracks_v1.2_live_parity",
...
def main() -> None:
    ...
    logging.info("Live-parity tracking: YOLO.track() is run on every decoded frame; only tracked outputs are sampled at %.3f FPS", args.sample_fps)
    ...
    total_tracked_frames = int(sum(int(s.get("tracked_frames") or 0) for s in all_video_stats))
    ...
    summary = {
        ...
        "pose_gate_note": (
            "This live-parity run is intended for the 5 FPS / 24-frame / stride-6 pose micro-kinematic gate. "
            "YOLO.track() is run on every decoded frame; only tracked outputs are sampled at sample_fps. "
            "Use this when evaluating the offline anomaly dataset against the deployed/live Pose gate."
        ),
    ...
    logging.info("Tracked decoded frames: %d", total_tracked_frames)
