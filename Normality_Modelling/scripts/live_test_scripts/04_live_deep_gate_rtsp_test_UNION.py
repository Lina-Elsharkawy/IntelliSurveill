#!/usr/bin/env python3
r"""
04_live_deep_gate_rtsp_test_UNION.py
...
Pipeline:
  ...
  4) Build 16-frame tubelets with stride 5 by default.
  5) Crop using a STATIC UNION crop across the whole tubelet by default.
...
Example:
  python 04_live_deep_gate_rtsp_test.py ^
    ...
    --device cuda ^
    --deep_gate_dir "D:\Embeddings_Distribution\normality_models\deep_gate\deep_branch_artifacts_v3_gaussian" ^
    --deep_k 5 ^
...
def pad_box_xyxy(
    ...
    """Pad an xyxy crop box and clip it to frame boundaries."""
    x1, y1, x2, y2 = [float(x) for x in bbox]
    bw = max(1.0, x2 - x1)
    bh = max(1.0, y2 - y1)
...
def union_box_xyxy(
    ...
    """Return one spatial union box covering all bboxes in a tubelet."""
    if not bboxes:
        return clip_bbox_xyxy((0, 0, 1, 1), width=width, height=height)
    xs1 = [float(b[0]) for b in bboxes]
    ys1 = [float(b[1]) for b in bboxes]
    xs2 = [float(b[2]) for b in bboxes]
    ys2 = [float(b[3]) for b in bboxes]
    return clip_bbox_xyxy(
        (min(xs1), min(ys1), max(xs2), max(ys2)),
        width=width,
        height=height,
    )
...
def crop_persons_rgb(
    samples: Sequence[Sample],
    crop_mode: str = "union",
    pad_ratio: float = 0.30,
    out_size: int = 224,
) -> Tuple[List[np.ndarray], Dict[str, float]]:
    """Crop tubelet frames for VideoMAE.

    union mode:
      - compute one union bbox across all 16 person boxes;
      - pad it once;
      - crop the exact same region from every frame;
      - resize every crop to 224x224 before VideoMAE.

    frame mode:
      - legacy behavior: crop each frame around its own bbox.
    """
    crops = []
    crop_areas = []
    pixel_means = []
    ...
    if mode not in {"union", "frame"}:
        raise ValueError(f"Unsupported crop_mode={crop_mode!r}; use 'union' or 'frame'.")

    union_crop_box = None
    if samples and mode == "union":
        h0, w0 = samples[0].frame_rgb.shape[:2]
        raw_union = union_box_xyxy([s.bbox_xyxy for s in samples], w0, h0)
        union_crop_box = pad_box_xyxy(raw_union, w0, h0, pad_ratio=pad_ratio)

    for s in samples:
        h, w = s.frame_rgb.shape[:2]
        if mode == "union":
            # Same crop box for every frame in the tubelet.
            # If frame size changes unexpectedly, clip again to the current frame.
            x1, y1, x2, y2 = clip_bbox_xyxy(union_crop_box, w, h)
        else:
            # Legacy fallback for A/B comparison with the old deep gate.
            x1, y1, x2, y2 = pad_box_xyxy(s.bbox_xyxy, w, h, pad_ratio=pad_ratio)

        crop = s.frame_rgb[y1:y2, x1:x2].copy()
        if crop.size == 0:
            crop = np.zeros((out_size, out_size, 3), dtype=np.uint8)
        else:
            crop = cv2.resize(crop, (int(out_size), int(out_size)), interpolation=cv2.INTER_LINEAR)
    ...
    stats = {
        ...
        "crop_out_size": int(out_size),
        "crop_union_x1": float(union_crop_box[0]) if union_crop_box is not None else float("nan"),
        "crop_union_y1": float(union_crop_box[1]) if union_crop_box is not None else float("nan"),
        "crop_union_x2": float(union_crop_box[2]) if union_crop_box is not None else float("nan"),
        "crop_union_y2": float(union_crop_box[3]) if union_crop_box is not None else float("nan"),
...
def process_tubelet(
    ...
    crops_rgb, crop_bbox_stats = crop_persons_rgb(
        tubelet_samples,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--det_conf", type=float, default=0.45)
    ...
    p.add_argument("--deep_k", type=int, default=5)
    ...
    p.add_argument("--bbox_pad_ratio", type=float, default=0.30)
    p.add_argument("--crop_mode", choices=["union", "frame"], default="union", help="union = static tubelet-level crop; frame = old per-frame moving crop")
    p.add_argument("--crop_size", type=int, default=224, help="Resize union/person crop to this square size before VideoMAE")
    ...
    p.add_argument("--max_track_gap_samples", type=int, default=2)
...
def summarize_settings(args: argparse.Namespace, deep_threshold: float, output_dir: Path, catastrophic_threshold: Optional[float] = None) -> Dict[str, Any]:
    return {
        "script": "04_live_deep_gate_rtsp_test_UNION.py",
        ...
        "notes": {
            "pipeline": [
                ...
                "Default crop_mode=union: one padded union crop is computed across the full tubelet and applied to all 16 frames.",
                "OpenCV BGR frames are converted to RGB before tubelet cropping.",
                "Each crop is resized to crop_size before VideoMAE to match offline extraction parity.",
