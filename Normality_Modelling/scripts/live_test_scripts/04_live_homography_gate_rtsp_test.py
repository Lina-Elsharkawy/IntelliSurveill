...
def compute_homography_macro_features(
    ...
    floor_points = project_points_homography(img_points, H)
    ...
    meta = {
        ...
        "floor_points": floor_points.tolist(),
        "step_speeds": speeds.astype(float).tolist(),
        "step_accels": accels.astype(float).tolist(),
...
def main():
    ...
    summary = {
        "script": "04_live_homography_gate_rtsp_test.py",
        ...
        "note": "Standalone homography macro-motion gate. Units are homography floor units, not guaranteed meters.",
    }
