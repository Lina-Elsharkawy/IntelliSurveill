...
# -*- coding: utf-8 -*-
r"""
04_live_homography_gate_rtsp_test_STAGE3_REBUILT_GATE_COOLDOWN.py

Standalone live RTSP tester for the HOMOGRAPHY / MACRO-MOTION gate with multiple real fixes plus event cooldown/rising-edge emission:
...
Event emission fix:
- Emit an event only on persistent-hit rising edge by default.
- Apply per-track event cooldown so one continuous episode does not create many duplicate events.
...
def _path_from_recommended(value: Any) -> Optional[Path]:
    if not value:
        return None
    try:
...
def load_gate_artifacts(args):
    """
    Load the rebuilt Stage-3 gate artifacts.

    Preferred source:
      homography_gate_dir/09_recommended_macro_gate.json

    This keeps the live script aligned with the exact threshold, scaler, optional PCA,
    and GMM produced by 04e_build_homography_macro_gmm_gate_STAGE3_POSE.py.
    CLI paths still override the JSON when supplied.
    """
    gate_dir = Path(args.homography_gate_dir)
    rec_path = Path(args.recommended_gate_json) if args.recommended_gate_json else (gate_dir / "09_recommended_macro_gate.json")
    rec = _read_json_if_exists(rec_path)

    artifacts = rec.get("artifacts", {}) if isinstance(rec, dict) else {}

    scaler_path = Path(args.scaler_path) if args.scaler_path else _path_from_recommended(artifacts.get("scaler"))
    pca_path = Path(args.pca_path) if args.pca_path else _path_from_recommended(artifacts.get("pca"))
    gmm_path = Path(args.gmm_path) if args.gmm_path else _path_from_recommended(artifacts.get("gmm"))

    if scaler_path is None:
        scaler_path = find_first_joblib(gate_dir, ["scaler"]) or find_first_joblib(gate_dir, ["robust"])
    if gmm_path is None:
        # Prefer the primary components used in the rebuilt gate, then fall back safely.
        primary_k = None
        if isinstance(rec, dict):
            primary_k = rec.get("primary_components") or rec.get("training", {}).get("primary_components")
        if primary_k:
            gmm_path = find_first_joblib(gate_dir, [f"components_{int(primary_k)}"])
        if gmm_path is None:
    ...
    if scaler_path is None or not scaler_path.exists():
        raise FileNotFoundError(
            "Could not find macro RobustScaler joblib. Pass --scaler_path explicitly or check 09_recommended_macro_gate.json."
        )
    if gmm_path is None or not gmm_path.exists():
        raise FileNotFoundError(
            "Could not find macro GMM joblib. Pass --gmm_path explicitly or check 09_recommended_macro_gate.json."
        )

    threshold = args.homography_threshold
    threshold_source = "--homography_threshold"
    if threshold is None:
        if isinstance(rec, dict) and rec.get("threshold") is not None:
            threshold = float(rec["threshold"])
            threshold_source = str(rec_path)
        elif isinstance(rec, dict) and rec.get("primary_threshold") is not None:
            threshold = float(rec["primary_threshold"])
            threshold_source = str(rec_path)
        else:
            raise ValueError(
                "No threshold was provided and no threshold was found in 09_recommended_macro_gate.json. "
                "Run 04e first or pass --homography_threshold explicitly."
            )

    pca = None
    if pca_path is not None and str(pca_path).strip() and pca_path.exists():
        pca = joblib.load(pca_path)
        print(f"[LOAD] pca={pca_path}")
    elif pca_path is not None and str(pca_path).strip():
        print(f"[WARN] PCA path was listed but not found, continuing without PCA: {pca_path}")

    selected_features = None
    if isinstance(rec, dict):
        selected_features = rec.get("selected_features") or rec.get("training", {}).get("selected_features")
        if selected_features and list(selected_features) != FEATURE_NAMES:
            print("[WARN] Gate selected_features differ from live FEATURE_NAMES:")
            print(f"       gate={selected_features}")
            print(f"       live={FEATURE_NAMES}")
            print("       Scores may be invalid unless the feature order is identical.")

    print(f"[LOAD] recommended_gate_json={rec_path if rec_path.exists() else 'not_found'}")
    print(f"[LOAD] scaler={scaler_path}")
    print(f"[LOAD] gmm={gmm_path}")
    print(f"[LOAD] threshold={float(threshold):.6f} source={threshold_source}")

    return (
        joblib.load(scaler_path),
        pca,
        joblib.load(gmm_path),
        str(scaler_path),
        str(pca_path) if pca_path else "",
        str(gmm_path),
        float(threshold),
        threshold_source,
    )
...
def parse_args():
    ...
    ap.add_argument("--homography_gate_dir", default=r"D:\Embeddings_Distribution\normality_models\homography_gate\homography_gate_stage3_pose\homography_macro_gmm_gate")
    ap.add_argument("--recommended_gate_json", default="", help="Optional path to 09_recommended_macro_gate.json. Defaults to homography_gate_dir/09_recommended_macro_gate.json")
    ap.add_argument("--scaler_path", default="")
    ap.add_argument("--pca_path", default="", help="Optional PCA joblib if 04e was trained with --use_pca")
    ap.add_argument("--gmm_path", default="")

    ap.add_argument("--homography_threshold", type=float, default=None, help="Override threshold. If omitted, load threshold from 09_recommended_macro_gate.json")
    ...
    # Event de-duplication / live alert hygiene.
    # The offline 04e evaluator uses min_event_gap_sec; live should not write a new event
    # for every tubelet while persistent_hit remains true.
    ap.add_argument("--min_event_gap_sec", type=float, default=8.0,
                    help="Minimum seconds between emitted events for the same track.")
    ap.add_argument("--emit_repeated_persistent_events", action="store_true",
                    help="Old behavior: emit an event for every tubelet with persistent_hit=True. Not recommended.")
...
def main():
    ...
    csv = CsvAppender(out_dir / "tubelets.csv", [
        ...
        *FEATURE_NAMES,
        ...
        "active_persistent_anomaly", "event_emitted", "persistent_rising_edge",
        "event_cooldown_ok", "seconds_since_last_event",
        "anomaly", "reasons", "evidence_frame", "evidence_montage",
    ])

    print("=" * 100)
    print("LIVE HOMOGRAPHY GATE - STAGE 3 REBUILT OFFLINE-COMPATIBLE GATE")
    ...
    scaler, pca, gmm, scaler_path, pca_path, gmm_path, active_threshold, threshold_source = load_gate_artifacts(args)
    ...
    states: Dict[int, OnlineGateState] = defaultdict(
        lambda: OnlineGateState(active_threshold, args.smoothing_sigma, args.persistence_hits, args.persistence_window)
    )
    ...
    # Event de-duplication state.
    # active_persistent_by_track stores whether the previous emitted tubelet for this track
    # was already in persistent anomaly state. This lets us detect a rising edge.
    active_persistent_by_track: Dict[int, bool] = defaultdict(bool)
    last_event_wall_time_by_track: Dict[int, float] = defaultdict(lambda: -1e18)
        ...
        while True:
            ...
            for box, det_conf, tid in zip(xyxy, confs, ids):
                ...
                if prev is not None and (sample_i - prev) > (args.max_track_gap_samples + 1):
                    ...
                    states[tid] = OnlineGateState(active_threshold, args.smoothing_sigma, args.persistence_hits, args.persistence_window)
                    ground_states[tid] = TrackGroundState()
                    active_persistent_by_track[tid] = False
                    last_event_wall_time_by_track[tid] = -1e18
                ...
                try:
                    ...
                    active_persistent_anomaly = bool(gr["persistent_hit"])
                    previous_persistent = bool(active_persistent_by_track.get(tid, False))
                    persistent_rising_edge = bool(active_persistent_anomaly and not previous_persistent)

                    seconds_since_last_event = float(now - last_event_wall_time_by_track[tid])
                    event_cooldown_ok = bool(seconds_since_last_event >= float(args.min_event_gap_sec))

                    if args.emit_repeated_persistent_events:
                        # Optional old behavior for debugging/comparison only.
                        event_emitted = active_persistent_anomaly and event_cooldown_ok
                    else:
                        # Recommended behavior: one event when the track enters persistent anomaly state,
                        # then suppress duplicates until the state clears and cooldown has elapsed.
                        event_emitted = persistent_rising_edge and event_cooldown_ok

                    if active_persistent_anomaly:
                        reasons.append("rare_macro_floor_motion")
                    if active_persistent_anomaly and not event_emitted:
                        if not persistent_rising_edge and not args.emit_repeated_persistent_events:
                            reasons.append("suppressed_same_persistent_episode")
                        if not event_cooldown_ok:
                            reasons.append("suppressed_event_cooldown")

                    # Update after computing rising edge.
                    active_persistent_by_track[tid] = active_persistent_anomaly

                    if event_emitted:
                        last_event_wall_time_by_track[tid] = float(now)

                    anomaly = bool(event_emitted)
                    feat_values = {name: float(X[0, i]) for i, name in enumerate(FEATURE_NAMES)}

                    label_state = "EVENT" if event_emitted else ("ACTIVE" if active_persistent_anomaly else "NORMAL")
                    latest_label[tid] = [
                        f"ID {tid} | {label_state}",
                        f"HOMO {score:.2f}/{active_threshold:.2f} S={gr['score_smooth']:.2f}",
                        f"R={int(gr['hit_raw'])} SH={int(gr['hit_smooth'])} P={int(gr['persistent_hit'])} E={int(event_emitted)}",
                    ...
                    if args.save_evidence and (event_emitted or active_persistent_anomaly or args.save_all_tubelets):
                    ...
                    row = {
                        ...
                        "macro_score": score,
                        "macro_threshold": float(active_threshold),
                        ...
                        "active_persistent_anomaly": active_persistent_anomaly,
                        "event_emitted": event_emitted,
                        "persistent_rising_edge": persistent_rising_edge,
                        "event_cooldown_ok": event_cooldown_ok,
                    ...
                    if event_emitted:
                        ...
                        event["event_policy"] = "persistent_rising_edge_with_per_track_cooldown"
                        event["min_event_gap_sec"] = float(args.min_event_gap_sec)
                        write_jsonl(events_jsonl, event)
                        print(f"[EVENT] {event['event_id']} track={tid} score={score:.3f} smooth={gr['score_smooth']:.3f} gp_valid={meta.get('valid_groundpoint_ratio', 0):.2f} cooldown={args.min_event_gap_sec:.1f}s reasons={reasons}")
                    elif args.print_every_tubelet:
                        print(
                            f"[TUBELET] #{tubelet_count} track={tid} "
                            f"score={score:.3f}/{active_threshold:.3f} "
                            f"smooth={gr['score_smooth']:.3f} "
                            f"raw={gr['hit_raw']} smooth_hit={gr['hit_smooth']} persist={gr['persistent_hit']} "
                            f"event={event_emitted} rising={persistent_rising_edge} cooldown_ok={event_cooldown_ok} "
    ...
    summary = {
        "script": "04_live_homography_gate_rtsp_test_STAGE3_REBUILT_GATE_COOLDOWN.py",
        ...
        "pca_path": pca_path,
        "gmm_path": gmm_path,
        "recommended_gate_json": str(Path(args.recommended_gate_json) if args.recommended_gate_json else (Path(args.homography_gate_dir) / "09_recommended_macro_gate.json")),
        "homography_threshold": float(active_threshold),
        ...
        "emit_repeated_persistent_events": bool(args.emit_repeated_persistent_events),
        "event_policy": "repeat_persistent_with_cooldown" if args.emit_repeated_persistent_events else "persistent_rising_edge_with_per_track_cooldown",
