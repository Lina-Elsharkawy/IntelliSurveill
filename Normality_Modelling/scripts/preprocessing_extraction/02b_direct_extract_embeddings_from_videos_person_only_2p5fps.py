#!/usr/bin/env python3
r"""
Direct PERSON-ONLY embedding extraction from raw surveillance videos.
...
FIXED VERSION for rebuilding the normal distribution dataset:
  ...
  - records actual temporal span metadata for every tubelet
  - PERSON-ONLY: context embeddings are not extracted or saved
...
Pipeline:
  ...
  -> quality-filter person tubelets -> VideoMAE embeddings -> .npy + CSV
...
Outputs:
  processed_dir/
    ...
    metadata/
      direct_rejected_tubelets.csv
      direct_invalid_embeddings.csv
    logs/
      direct_embeddings.log
    debug_tubelets/              optional, only if --debug_save_mp4_limit > 0
      person/*.mp4

Run from the project root, for example:
  python scripts\02b_direct_extract_embeddings_from_videos.py ^
    --input_dir "D:\Embeddings_Distribution\raw_videos" ^
    --processed_dir "D:\Embeddings_Distribution\processed_dataset_direct_2p5fps_6sec" ^
...
DEFAULT_PROCESSED_DIR = Path(r"D:\Embeddings_Distribution\processed_dataset_direct_2p5fps_6sec")
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("direct_embeddings")
...
class TrackBuffer:
    ...
    person_frames: List[np.ndarray] = field(default_factory=list)
    ...
    def latest_window(self, tubelet_frames: int) -> Dict[str, Any]:
        return {
            ...
            "person_frames": self.person_frames[-tubelet_frames:],
...
class DirectExtractor:
    def __init__(self, args: argparse.Namespace, logger: logging.Logger) -> None:
        ...
        self.metadata_csv = self.emb_dir / "direct_embedding_metadata.csv"
        self.rejected_csv = self.meta_dir / "direct_rejected_tubelets.csv"
        self.invalid_csv = self.meta_dir / "direct_invalid_embeddings.csv"
    ...
    def enqueue_tubelet(self, meta: Dict[str, Any], person_frames: List[np.ndarray]) -> None:
        ...
        if self.args.debug_save_mp4_limit > 0 and self.debug_saved < self.args.debug_save_mp4_limit:
            write_mp4(person_frames, self.debug_dir / "person" / f"{tubelet_id}.mp4", self.args.sample_fps)
            self.debug_saved += 1
        ...
        while True:
            ...
            for det_i, tid in enumerate(ids):
                ...
                if bbox[2] < self.args.min_bbox_width or bbox[3] < self.args.min_bbox_height:
                    continue
                try:
                    person_crop = crop_resize(frame, pad_bbox(bbox, self.args.person_padding, frame.shape), self.args.person_size)
                except Exception:
                    continue
                ...
                if buf.can_emit(self.args.tubelet_frames, self.args.stride, sample_idx):
                    ...
                    if ok:
                        ...
                        self.enqueue_tubelet(meta, list(win["person_frames"]))
    ...
    def run(self) -> None:
        ...
        if not videos:
            ...
            return
        self.logger.info(f"Found {len(videos)} video(s). Starting direct embedding extraction.")
        total_acc = 0
        total_rej = 0
        ...
        self.flush_batch()
        self.save_checkpoint()
        save_json(self.emb_dir / "direct_extraction_summary.json", {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Direct PERSON-ONLY embedding extraction from raw videos")
    ...
    parser.add_argument("--person_padding", type=float, default=0.20, help="Person bbox padding")
...
def validate_output_destination(args: argparse.Namespace) -> None:
    ...
    existing_outputs = [
        ...
        emb_dir / "direct_embedding_metadata.csv",
    ]
...
def main() -> None:
    ...
    logger = setup_logger(args.processed_dir / "logs" / "direct_embeddings.log")
    ...
    logger.info("PERSON-ONLY MODE: context crops/embeddings are disabled; only person_embeddings.npy will be saved.")
