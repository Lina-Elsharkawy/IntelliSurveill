...
# -*- coding: utf-8 -*-
r"""
Deep Gate v4 — anomaly dataset FULL all-k hyperparameter-sweep plots and LaTeX tables
...
python "D:\Embeddings_Distribution\scripts\deep_gate\plot_v4_anomaly_all_k_sweep_fixed.py" --csv "D:\Embeddings_Distribution\anomaly_dataset\outputs\deep_eval_v4_liveparity_2p5fps_16f_s8\reports\06_deep_eval_config_sweep.csv" --summary-json "D:\Embeddings_Distribution\anomaly_dataset\outputs\deep_eval_v4_liveparity_2p5fps_16f_s8\06_deep_eval_summary.json" --embedding-summary-json "D:\Embeddings_Distribution\anomaly_dataset\outputs\deep_from_motion_tubelets_liveparity_2p5fps_16f_s8\embeddings\deep_from_motion_tubelets_summary.json" --outdir "D:\Embeddings_Distribution\anomaly_dataset\outputs\deep_eval_v4_liveparity_2p5fps_16f_s8\plots\all_k_hyperparam_sweep_fixed" --normal-tubelets 1274 --anomaly-tubelets 1009
"""
...
DEFAULT_CSV = Path(
    ...
    r"\deep_eval_v4_liveparity_2p5fps_16f_s8"
...
DEFAULT_OUTDIR = Path(
    r"D:\Embeddings_Distribution\anomaly_dataset\outputs"
    r"\deep_eval_v4_liveparity_2p5fps_16f_s8"
    r"\plots\all_k_hyperparam_sweep_fixed"
)
...
def plot_dataset_composition(
    ...
    fig.suptitle(
        f"Deep Gate v4 anomaly-evaluation dataset composition{subtitle}",
        fontsize=13,
        fontweight="bold",
    )
...
def plot_auc_auprc_grouped(
    ...
    fig.suptitle(
        "Tubelet-level ranking performance before thresholding and persistence",
        fontsize=13,
        fontweight="bold",
    )
    fig.savefig(outpath, dpi=300, bbox_inches="tight")
...
def plot_metric_vs_threshold_all_k(
    ...
    x-axis = threshold percentile.
    Lines = score modes.
    The empty sixth panel is used for the legend, so the title is not hidden.
    """
    ...
    # Use the sixth axis as a legend box.
...
def plot_counts_vs_threshold_all_k(
    ...
    """
    Plot TP, FP, TN, FN for ALL k values.
    Rows = k.
    Columns = TP/FP/TN/FN.
    Lines = score modes.
    Metrics are video-level after thresholding and persistence.
    """
    k_values = sorted(df["k"].unique())
    mode_order = df.attrs["mode_order"]
    threshold_order = df.attrs["threshold_order"]
    ...
    fig.suptitle(
        "Video-level TP/FP/TN/FN counts after thresholding and persistence",
        fontsize=14,
        fontweight="bold",
        y=0.995,
    )
...
def plot_operating_point_scatter(
    ...
    # To keep the legend readable, label only by k marker and score mode colour separately.
    mode_handles = []
    k_handles = []
    ...
    ax.set_title(
        "Operating-point trade-off after thresholding and persistence",
        fontsize=13,
        fontweight="bold",
    )
...
def plot_metric_heatmap_grid(
    ...
    fig.suptitle(f"Deep Gate v4 {label} after thresholding and persistence", fontsize=14, fontweight="bold")
...
def write_summary_tables(df: pd.DataFrame, outdir: Path) -> None:
    ...
    table_df.to_csv(tables_dir / "v4_anomaly_all_configurations_metrics.csv", index=False)

    dataframe_to_latex_longtable(
        table_df,
        caption=(
            "Deep Gate v4 anomaly-dataset evaluation across all hyperparameter "
            "configurations. Video-level metrics are computed after thresholding "
            "and persistence."
        ),
        label="tab:deep_v4_anomaly_all_configs",
        outpath=tables_dir / "v4_anomaly_all_configurations_metrics_longtable.tex",
    )
    ...
    for k in sorted(df["k"].unique()):
        ...
        dataframe_to_latex_table(
            k_table,
            caption=(
                f"Deep Gate v4 anomaly-dataset evaluation for k={int(k)} "
                "across all score modes and thresholds. Video-level metrics are "
                "computed after thresholding and persistence."
            ),
            label=f"tab:deep_v4_anomaly_k{int(k)}",
            outpath=tables_dir / f"v4_anomaly_metrics_k{int(k)}.tex",
        )
    ...
    ranked.to_csv(tables_dir / "v4_anomaly_configurations_ranked_by_f1.csv", index=False)

    zero_fp = df[df["video_fp"] == 0].copy()
    zero_fp[compact_cols].sort_values(
        ["video_recall", "video_f1"], ascending=False
    ).to_csv(tables_dir / "v4_anomaly_zero_fp_configurations.csv", index=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Generate Deep Gate v4 anomaly-dataset FULL all-k hyperparameter sweep "
    ...
    print("=" * 80)
    print("Deep Gate v4 anomaly ALL-K hyperparameter-sweep plotting")
    print("=" * 80)
    ...
    plot_dataset_composition(
        ...
        df=df,
        outpath=args.outdir / "FIG_v4_anomaly_01_dataset_composition.png",
    ...
    plot_auc_auprc_grouped(
        df=df,
        outpath=args.outdir / "FIG_v4_anomaly_02_tubelet_auroc_auprc_all_k.png",
        show=args.show,
    )

    plot_metric_vs_threshold_all_k(
        df=df,
        outpath=args.outdir / "FIG_v4_anomaly_03_video_recall_vs_threshold_ALL_K.png",
        metric="video_recall",
        metric_label="Video recall",
        title="Video recall after thresholding and persistence for ALL k values",
        y_max=0.55,
        show=args.show,
    )
    ...
    plot_metric_vs_threshold_all_k(
        df=df,
        outpath=args.outdir / "FIG_v4_anomaly_04_video_f1_vs_threshold_ALL_K.png",
        metric="video_f1",
        metric_label="Video F1",
        title="Video F1 after thresholding and persistence for ALL k values",
        y_max=0.65,
        show=args.show,
    )
    ...
    plot_metric_vs_threshold_all_k(
        df=df,
        outpath=args.outdir / "FIG_v4_anomaly_05_normal_video_fpr_vs_threshold_ALL_K.png",
        metric="video_normal_video_false_positive_rate",
        metric_label="Normal-video false-positive rate",
        title="Normal-video FPR after thresholding and persistence for ALL k values",
        y_max=0.30,
        show=args.show,
    )
    ...
    plot_counts_vs_threshold_all_k(
        df=df,
        outpath=args.outdir / "FIG_v4_anomaly_06_tp_fp_tn_fn_counts_vs_threshold_ALL_K.png",
        show=args.show,
    )

    plot_operating_point_scatter(
        df=df,
        outpath=args.outdir / "FIG_v4_anomaly_07_operating_point_tradeoff_ALL_CONFIGS.png",
        show=args.show,
    )
    ...
    if args.heatmaps:
        ...
        for metric, label, fmt in heatmap_specs:
            plot_metric_heatmap_grid(
                ...
                metric=metric,
                label=label,
                outpath=heatmap_dir / f"FIG_v4_anomaly_heatmap_{metric}_ALL_K.png",
                fmt=fmt,
                show=args.show,
            )
    ...
    for f in sorted(args.outdir.glob("FIG_v4_anomaly_*.png")):
        print(f"  {f}")

    if args.heatmaps:
        print()
        print("Created heatmaps:")
        for f in sorted((args.outdir / "heatmaps").glob("FIG_v4_anomaly_heatmap_*.png")):
            print(f"  {f}")
